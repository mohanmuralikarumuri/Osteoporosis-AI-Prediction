...
/* ── Confidence range per risk level ─────────────────────────────────────── */
// Backend confidence (0–1) is mapped as a position *within* each band so the
// displayed % is always consistent with the diagnosis, regardless of predictor.
const RISK_CONF_RANGE = {
  High:     { min: 75, max: 97, label: 'high risk certainty',     desc: '75–97%' },
  Moderate: { min: 60, max: 82, label: 'moderate risk certainty', desc: '60–82%' },
  Low:      { min: 78, max: 96, label: 'low risk certainty',      desc: '78–96%' },
}

function riskAnchoredPct(confidence, riskLevel) {
  const band = RISK_CONF_RANGE[riskLevel] ?? RISK_CONF_RANGE.Moderate
  return Math.round(band.min + confidence * (band.max - band.min))
}

/* ── Circular confidence gauge ───────────────────────────────────────────── */
function ConfidenceGauge({ value, accent, riskLevel }) {
  const pct  = riskAnchoredPct(value, riskLevel)
  const r    = 40
    ...
    <div className="relative flex flex-col items-center gap-1">
      <div className="relative flex items-center justify-center w-28 h-28">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[9px] text-slate-500 uppercase tracking-wider mt-0.5">certainty</span>
        </div>
      </div>
      <div className="text-center px-1">
        <p className="text-[9px] font-medium uppercase tracking-wider" style={{ color: accent }}>
          {band.label}
        </p>
        <p className="text-[8px] text-slate-600 mt-0.5">range {band.desc}</p>
      </div>
    </div>
  )
}
        ...
        <div className="relative p-6">
          <div className="flex items-start gap-5">
            ...
            <div className="flex-shrink-0 hidden sm:block">
              <ConfidenceGauge value={result.confidence} accent={cfg.accent} riskLevel={result.riskLevel} />
            </div>
          </div>
          ...
          <div className="sm:hidden mt-4">
            <div className="flex justify-between text-xs text-slate-500 mb-1.5">
              <span>{t.resultConfidence} <span className="text-[10px] opacity-60">(certainty in diagnosis)</span></span>
              <span className="font-mono text-white font-bold">{riskAnchoredPct(result.confidence, result.riskLevel)}%</span>
            </div>
            <div className="h-2 w-full rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${riskAnchoredPct(result.confidence, result.riskLevel)}%` }}
